...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val list = grades.toList()
    return list.groupBy({ it.second }, { it.first })
}
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    mapB.forEach {
        ...
        value = if ((value == null) or (value == it.value)) it.value else value + ", ${it.value}"
        result[it.key] = value
    }
    return result
}
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for ((k, v) in friends) {
        val q = v.toMutableList()
        var i = 0
        ans[k] = mutableSetOf()
        while (i < q.size) {
            if (q[i] in friends) {
                for (friend in friends[q[i]]!!.toList()) if (friend !in ans[k]!! && friend != k) q.add(friend)
            } else {
                ans[q[i]] = mutableSetOf()
            }
            ans[k]!!.add(q[i])
            i++
        }
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (j in 0 .. n - 1) {
        if ((number - list[j] in index) && (j != index[number - list[j]])) {
            return j to index[number - list[j]]!!
        }
    }
    return -1 to -1
}
